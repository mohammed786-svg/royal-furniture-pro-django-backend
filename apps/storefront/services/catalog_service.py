...
class StorefrontCatalogService:
    ...
    def _build_listing(
        ...
        *,
        ...
        category = category_repository.fetch_by_slug(category_slug)
        if not category:
            raise NotFoundException("Category not found")

        sub = sub_category_repository.fetch_by_slug(int(category["category_id"]), sub_category_slug)
        if not sub:
        ...
        rows, total = product_repository.list_storefront_paginated(
            ...
            sub_category_id=int(sub["sub_category_id"]),
            sort_by=sort_key,
        )

        cat_slug = from_db_text(category.get("slug")) or category_slug
        sub_slug = from_db_text(sub.get("slug")) or sub_category_slug
        under_rows = storefront_catalog_repository.list_under_sub_categories(int(sub["sub_category_id"]))
        ...
        version = f"{float(category.get('epoch') or 0):.6f}-{float(sub.get('epoch') or 0):.6f}-{total}"

        return {
            "department": from_db_text(category.get("name")) or "",
            "category": from_db_text(sub.get("name")) or "",
            "title": from_db_text(sub.get("name")) or "",
    ...
    def get_category_listing(
        ...
        *,
        ...
        return cache_manager.get_or_set(
            CacheKeys.storefront_plp(category_slug, sub_category_slug, page, sort_key),
            loader,
